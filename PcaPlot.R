...
PcaPlot<-function(ExpMat,GroupFile,transcript=FALSE,batch=FALSE,log=FALSE,Directory,features='ExperimentalFeatures',tissue='TissueUsedInAnalysis'){
	...
	if(batch){
		...
		Dis<-(unique(as.vector(labels)))
		LabCol<-rainbow(length(Dis[!(Dis=='REF')]))
		...
		plot(exp.pca$x[,1], exp.pca$x[,2], col=labels,pch=c(as.numeric(size)), main=paste("PCA1vs2_", features, tissue,sep=""), xlab = "PCA 1", ylab = "PCA 2")
		...
		plot(exp.pca$x[,2], exp.pca$x[,3], col=labels, pch=c(as.numeric(size)), main=paste("PCA2vs3_", features, tissue,sep=""), xlab = "PCA 2", ylab = "PCA 3")
		text(exp.pca$x[,2], exp.pca$x[,3], labels=na, pos= 3) #labels points
		legend("bottomright",legend=paste(Dis,sep=''),fill=paste(LabCol,sep=''))
		legend("bottomleft",legend=paste(batch,sep=''),pch=as.numeric(shape))
		plot(exp.pca$x[,3], exp.pca$x[,4], col=labels, pch=c(as.numeric(size)), main=paste("PCA3vs4_", features, tissue,sep=""), xlab = "PCA 3", ylab = "PCA 4")
